...
#		debug($newArray);
	...
	function mergeFormDataIntoTemplateStructure($editStruct,$currentMappingInfo,$firstLevelImplodeToken='',$valueKey='vDEF')	{
		...
		if (is_array($editStruct))	{
			...
			//#
			//### Mansoor Ahmad - PHP 5.3 Compatible Hack
			//#
			$isSection = (preg_match('/^[0-9]/',$testInt))?TRUE:FALSE;
		}
		$out='';
		if ($isSection)	{
	...
	function splitPath($pathStr)	{
		...
		foreach($subPaths as $index => $path)	{
			...
			//#
			//### Mansoor Ahmad - Change of PHP5.3 support
			//#
	...
	function sourceDisplay($str,$recursion,$gnyf='',$valueStr=0)	{
		if (strcmp(trim($str),''))	{
			return str_pad('',$recursion*2,' ',STR_PAD_LEFT).
				...
				//#
				//### Mansoor Ahmad - Change of PHP5.3
				//#
				htmlspecialchars(t3lib_div::fixed_lgd(preg_replace('/[[:space:]]+/',' ',$str),$this->maxLineLengthInSourceMode)).
				($valueStr ? '</em></font>' : '').
				chr(10);
		}
		return '';
