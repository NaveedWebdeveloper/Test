		...
		switch ($contentTreeArr['el']['table']) {
			...
			case 'tt_content' :
				...
				if (!$this->translatorMode && $canEditContent) {
					...
					$linkUnlink = $this->link_unlink('<img'.t3lib_iconWorks::skinImg($this->doc->backPath,'gfx/garbage.gif','').' title="'.$LANG->getLL('unlinkRecord').'" border="0" alt="" />', $parentPointer, ($elementBelongsToCurrentPage ? TRUE : FALSE));
			...
			if (($commandParameters = t3lib_div::_GP($command)) != '') {
				...
				switch ($command) {
					...
					case 'deleteRecord':
						...
						// addad by Caspar @ Gosign media <caspar@gosign.de>
						// delete CEs recursive, if there are child-elements
						$getDeleteRecord = $this->apiObj->flexform_getRecordByPointer($commandParameters);
						$recursiveDeleteDestinationPointerArray = $this->apiObj->flexform_getFlexformPointersToSubElementsRecursively('tt_content', $getDeleteRecord['uid'], $dummyArray = array());
						while($recursiveDeleteDestinationPointer = array_pop($recursiveDeleteDestinationPointerArray)) {
							$getRecursiveDeleteRecord = $this->apiObj->flexform_getRecordByPointer($recursiveDeleteDestinationPointer);
							if($getRecursiveDeleteRecord['pid'] == $this->id) {
								$this->apiObj->deleteElement($recursiveDeleteDestinationPointer);
							}
							else {
								$this->apiObj->unlinkElement($recursiveDeleteDestinationPointer);
							}
						}
						// end of add
